...
int int_to_string(int i, char str[], int d) {
	int digit, count, length = get_int_length(i);                   //Member declaration, get length of input integer
...
void double_to_string(double f, char* str, int resolution) {
	int integerPart, length, isNegative;                                        //Member variables
	double floatingPart;
	
	if(f < 0) {isNegative = 1;}

	integerPart = (int)f;                                           //Extract integer part
	floatingPart = f - (double)integerPart;                          //Extract floating part (after decimal point)
